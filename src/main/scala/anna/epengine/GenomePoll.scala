 ...
 */
class GenomePoll(genomes: List[NetGenome]){

}

object GenomePoll {
  def apply(netId: String, inputIds: List[String], outputIds: List[String], size: Int) = {
    ...
    assert(inputIds.size + outputIds.size <= NetGenome.neuronsRange.end, s"You chose ${inputIds.size} inputs and ${outputIds.size} outputs, but the max possible neurons number is only ${neuronsRange.end}")
