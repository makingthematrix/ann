...
case class SignalSum(name: String, requiredSignalsNumber: Int){
  ...
  def chain(builder: NetBuilder, inputWeight: Double = 1.0, inputThreshold: Double = 0.0) = {
    ...
    if(builder.isCurrent) builder.chain(inputId, inputWeight, inputThreshold, SilenceIterations(0))
    else builder.addMiddle(id=inputId, threshold=0.0, silenceIterations = SilenceIterations(0))

    builder.use(inputId).chain(outputId, middleSynapseWeigth, middleThreshold, SilenceIterations(0))
