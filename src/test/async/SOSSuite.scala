...
class SOSSuite extends MySuite {
  ...
  private def firstTry(){
    ...
    builder.use("in1").chainMiddle("mi21",0.13,0.5).chainMiddle("mi22",1.0,0.5).chainMiddle("line",1.0).chainMiddle("O",0.9)
    builder.use("mi21").setForgetting(0.02)
    builder.use("O").hush("S").hush("dot")    
  }  
  ...
  private def secondTry(){
    ...
    // dots
    builder.use("in1").chainMiddle("mi11",0.6,0.5,HushValue(-1.0),ForgetAll).loop("loop1",1.0,0.5,1.0).chainMiddle("mi12",1.0,0.75).chainMiddle("dot",1.0).chainMiddle("S",0.5,0.9)
    builder.use("dot")/*.hush("mi11")*/.hush("loop1").hush("mi12")//.connect("mi11", -0.49).connect("mi12", -1.0)
         
    // lines
    builder.use("in1").chainMiddle("mi21",0.4,0.6,HushValue(-1.0)).chainMiddle("mi22",1.0,0.6).chainMiddle("line",1.0).chainMiddle("O",0.9,0.9)
  ...
  private def thirdTry(){
    ...
    builder.addInput("in1")
    // dots
    builder.use("in1").chainMiddle("mi11",0.6,0.5,HushValue(-1.0),ForgetAll)
                      .loop("loop1",1.0,0.5,1.0)
                      .chainMiddle("mi12",1.0,0.75,HushValue(-1.0))
                      .chainMiddle("dot",1.0)
                      .chainMiddle("S",0.5,0.9)
    builder.use("dot").hush("loop1").hush("mi12")
         
    // lines
    builder.use("in1").chainMiddle("mi21",0.4,0.6,HushValue(-1.0),ForgetValue(0.07))
  ...
  @Test def shouldHaveDotRes1() = {
    ...
    builder.addInput("in")
    // dots
    builder.use("in").chainMiddle("mi11",1.0,0.7,HushValue(-1.0),ForgetValue(0.5)).hush("mi11")
