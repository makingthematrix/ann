...
class Net(val defSlope: Double = 20.0,val defThreshold: Double = 0.5, val defWeight: Double = 1.0) extends AbstractNet[Neuron] {
  ...
  def addNeuron(slope: Double =defSlope, treshold: Double =defThreshold): Neuron = {
    val n = Neuron(treshold,slope)
    neurons += n
    n
  }
  def addNeuron(neuron: Neuron){
    LOG.debug(this, "adding neuron " + neuron.id)
    neurons += neuron
  }
